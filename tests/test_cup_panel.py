...
возвращал невыполненную корутину вместо числа рынков, а кнопки публикации
теряли действие при разборе `cup_post_line_5` по первому «_».
"""
...
class CupPanelCallbackTest(unittest.TestCase):
    ...
    def test_publish_buttons_reach_their_action(self):
        publish = AsyncMock(return_value="опубликовано")
        _, render, _ = self._run("cup_post_line_5", _publish=publish)
        publish.assert_awaited_once()
        self.assertTrue(publish.await_args.kwargs["line"])
        self.assertEqual(render.await_args.kwargs["note"], "опубликовано")

        publish.reset_mock()
        self._run("cup_post_results_5", _publish=publish)
        publish.assert_awaited_once()
        self.assertFalse(publish.await_args.kwargs["line"])
