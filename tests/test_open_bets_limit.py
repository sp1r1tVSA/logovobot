...
class TestLimitEnforcement(OpenBetsLimitCase):
    """Купон сверх лимита не принимается, пока не рассчитан один из открытых."""

    def test_default_ceiling_is_twelve(self):
        self.assertEqual(DEFAULT_MAX_OPEN_BETS, 12)
        self.assertEqual(BettingLimitsService.get_system_limits()["max_open_bets"], 12)

    def test_coupon_over_the_limit_is_rejected(self):
        for n in range(LIMIT):
            ok, _ = self._place(n)
            self.assertTrue(ok, f"купон #{n + 1} должен был пройти")

        ok, err = self._place(LIMIT)
        self.assertFalse(ok)
        self.assertIsInstance(err, dict)
        self.assertEqual(err["error"], "OPEN_BETS_LIMIT")
        self.assertEqual(err["max_open_bets"], LIMIT)
        self.assertEqual(err["open_bets"], LIMIT)
        self.assertIn(str(LIMIT), err["message"])
    ...
    def test_rejected_coupon_costs_nothing(self):
        ...
        ok, _ = self._place(LIMIT)
        self.assertFalse(ok)
        self.assertEqual(database.get_or_create_wallet(self.user_id)["balance"], balance_before)
        self.assertEqual(database.get_user_open_bets_count(self.user_id), LIMIT)
    ...
    def test_a_settled_coupon_reopens_the_slot(self):
        ...
        self.assertFalse(self._place(LIMIT)[0])
        self._settle(bet_ids[0], "won")
        self.assertTrue(self._place(LIMIT)[0], "после расчёта слот должен освободиться")

    def test_an_express_takes_a_single_slot(self):
        """Пять матчей в одном купоне — один слот, а не пять."""
        ok, _ = self._place(0, 1, 2, 3, 4)
        self.assertTrue(ok)
        last = 5 + LIMIT - 1
        for n in range(5, last):
            self.assertTrue(self._place(n)[0], f"ординар #{n} должен был пройти")
        self.assertFalse(self._place(last)[0])
    ...
    def test_old_coupons_count_but_are_not_cancelled(self):
        ...
        self.assertEqual(database.get_user_open_bets_count(self.user_id), LIMIT + 2)

        ok, err = self._place(0)
        self.assertFalse(ok)
        self.assertEqual(err["open_bets"], LIMIT + 2)
        ...
        self.assertEqual(still_open, LIMIT + 2)
...
class TestLimitHierarchy(OpenBetsLimitCase):
    ...
    def test_user_override_can_raise_the_ceiling_above_the_default(self):
        raised = LIMIT + 2
        BettingLimitsService.set_limit("global", 0, "max_open_bets", raised)
        BettingLimitsService.set_limit("user", self.user_id, "max_open_bets", raised)
        for n in range(raised):
            self.assertTrue(self._place(n)[0], f"купон #{n + 1} должен был пройти")
        self.assertFalse(self._place(raised)[0])
    ...
    def test_the_stricter_of_user_and_global_wins(self):
        BettingLimitsService.set_limit("global", 0, "max_open_bets", LIMIT + 4)
